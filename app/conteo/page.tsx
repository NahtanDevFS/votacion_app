...
type Opcion = { nombre: string; imagen?: File | null; imagen_url?: string | null; preview?: string };
...
export default function ConteoPage() {
  ...
  const { error } = await supabase.from("votacion").update({ estado: nuevoEstado }).eq("id", v.id);
 ...
 const handleDeleteVotacion = async (id: number) => {
  ...
  const handleOpcionChange = (index: number, value: string) => {
    ...
    setNewVotacion({ ...newVotacion, opciones: nuevas, opcionesConImagen: conImg });
  };

  const handleImageChange = (index: number, file: File | null) => {
    const nuevas = [...newVotacion.opcionesConImagen];
    if (!nuevas[index]) nuevas[index] = { nombre: newVotacion.opciones[index] || "" };
    nuevas[index] = {
      ...nuevas[index], imagen: file, preview: file ? URL.createObjectURL(file) : undefined
    };
  ...
  const handleRemoveImage = (index: number) => {
    ...
    if (nuevas[index]) {
      nuevas[index] = {
        ...nuevas[index], imagen: null, imagen_url: null, preview: undefined
      };
    }
    setNewVotacion({ ...newVotacion, opcionesConImagen: nuevas });
  };
...
const handleUpdateVotacion = async () => {
  ...
  try {
    ...
    await supabase.from("opcion_votacion").delete().eq("votacion_id", infoVotacion.id);
    ...
    {infoVotacion && (
      <div className="info-votacion-extra">
        ...
        <div className="info-textos">
          ...
          <p><strong>Código:</strong> {infoVotacion.token_link}</p>
          <p><strong>Estado:</strong> {infoVotacion.estado === "en_progreso" ? "En progreso" : "Expirada"}</p>
          <div className="botones-accion">
            <button className="btn-accion" onClick={() => handleEditClick(infoVotacion)}>✏️ Editar</button>
            <button className="btn-accion" onClick={() => handleToggleState(infoVotacion)}>❌ Cambiar estado</button>
            <button
              className="btn-accion"
              onClick={() => handleDeleteVotacion(infoVotacion.id)}
              disabled={deletingId === infoVotacion.id}
            >
              {deletingId === infoVotacion.id ? "Eliminando..." : "🗑️ Eliminar"}
            </button>
          </div>
        </div>
      </div>
            ...
            {newVotacion.opciones.map((op, index) => (
              <div key={index} className="opcion-input-container">
                <div className="opcion-input">
                  ...
                  />
                  {index > 0 && (
                    <button onClick={() => handleRemoveOpcion(index)}>✕</button>
                  )}
                </div>
